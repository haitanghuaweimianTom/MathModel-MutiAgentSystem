...
# 强制Windows下使用UTF-8编码，解决中文乱码问题
if sys.platform == "win32":
    import asyncio
    asyncio.set_event_loop_policy(asyncio.WindowsSelectorEventLoopPolicy())
    # 设置标准输出/错误输出的编码为UTF-8
    try:
...
def _find_claude_code() -> Optional[str]:
    ...
    from ..config import get_settings
    settings = get_settings()
    local_source = settings.claude_code_source_path
    if local_source and os.path.isdir(local_source):
        cli_js = os.path.join(local_source, "cli.js")
        if os.path.isfile(cli_js):
            # 使用系统 Node.js（已在PATH中）运行 cli.js
            _CLAUDE_CODE_PATH = f"node \"{cli_js}\""
            return _CLAUDE_CODE_PATH
...
def _call_claude_code_agent(
    ...
    if allowed_tools:
        extra_args.extend(["--allowedTools", ",".join(allowed_tools)])
