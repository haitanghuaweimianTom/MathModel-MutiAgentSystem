...
class AnalyzerAgent(BaseAgent):
    ...
    def get_system_prompt(self) -> str:
        return """你是一个专业的数学建模分析师。你的职责是：
1. 理解用户给出的数学建模问题
2. 识别问题的类型（优化/预测/评价/分类/仿真/网络/综合问题）
3. 将问题分解为若干个子问题（每个"要求"或"问题"对应一个子问题）
4. 为每个子问题推荐合适的求解方法

重要提示：
- 数模赛题通常包含多个子问题（用（1）（2）（3）或"要求1：/问题1："等标记）
- 每个子问题应单独分析，包括其具体问题类型和推荐方法
- 如果题目说"要求：（1）建模...（2）预测...（3）评价..."，则每个要求对应一个子问题
- 综合问题（如同时包含优化+预测+评价）应识别为"综合问题"

重要：你必须以JSON格式输出，不要有任何其他文字！

输出格式：
{
    "problem_type": "优化/预测/评价/分类/仿真/网络/综合问题",
    "difficulty": "简单/中等/困难",
    "overall_approach": "一句话描述总体分析思路",
    "sub_problems": [
        {
            "id": 1,
            "name": "子问题名称（如：最优订货量模型）",
            "description": "该子问题的核心描述",
            "problem_type": "该子问题的类型（优化/预测/评价等）",
            "approach": "求解思路",
            "suggested_method": "推荐的具体方法"
        }
    ],
...
请确保sub_problems数组包含所有子问题！"""
    ...
    async def execute(self, task_input: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 第二步：提取子问题（先用正则，关键预提取）
        extracted_tasks = _extract_sub_problems(problem_text)
        logger.info(f"AnalyzerAgent: 正则提取到 {len(extracted_tasks)} 个子问题")

        # 第三步：调用LLM深度分析
        messages = [
            {"role": "system", "content": self.get_system_prompt()},
            {"role": "user", "content": f"""请分析以下数学建模问题，严格识别所有子问题：

{problem_text}

注意：
1. 如果题目有多个"要求"或"问题"（如"要求：（1）...（2）..."），每个对应一个子问题
2. 如果没有明确编号，根据语义分解问题
3. 输出完整的JSON，不要遗漏任何子问题"""},
        ]
        ...
        try:
            ...
            if start != -1 and end > start:
                ...
                # 如果LLM只返回1个子问题但正则提取到了多个，用正则结果补充
                llm_subs = result.get("sub_problems", [])
                if len(llm_subs) == 1 and len(extracted_tasks) > 1:
                    logger.info(f"AnalyzerAgent: LLM只返回{len(llm_subs)}个子问题，但正则提取到{len(extracted_tasks)}个，使用正则结果补充")
                    result["sub_problems"] = _build_sub_problems_from_tasks(
                        extracted_tasks, result, keyword_results
                    )
                ...
                if keyword_results:
                    ...
                    if not result.get("problem_type"):
                        result["problem_type"] = top[1]
                    result["_keyword_top_type"] = top[0]
                    result["_keyword_methods"] = top[3] if len(top) > 3 else []

                logger.info(f"AnalyzerAgent 完成: {result.get('problem_type', 'unknown')}, {len(result.get('sub_problems', []))}个子问题")
                return result
            else:
                logger.warning(f"AnalyzerAgent: LLM未返回有效JSON，使用正则分析")
        except Exception as e:
            logger.warning(f"AnalyzerAgent LLM解析失败: {e}，使用正则+关键词分析")

        # 第四步：正则+关键词分析兜底
        result = self._keyword_fallback(problem_text, keyword_results, extracted_tasks)
        logger.info(f"AnalyzerAgent 关键词兜底: {result.get('problem_type')}, {len(result.get('sub_problems', []))}个子问题")
        return result
